...
export const AlertProvider = ({ children }) => {
    ...
    if (!attachedRef.current) {
      ...
      socket.on('dashboard_alert', (data) => {
        const normalized = {
          ...
          severity:     data.severity     || (data.type === 'EMERGENCY' ? 'Emergency' : 'Warning'),
      ...
      socket.on('dashboard_alert_clip', ({ _id, clipPath }) => {
        ...
        setAlerts(prev =>
          prev.map(a => String(a._id) === String(_id) ? { ...a, clipPath } : a)
        );
      });
  ...
  const acknowledgeAlert = useCallback((_id) => {
    setAlerts(prev =>
      prev.map(a => String(a._id) === String(_id) ? { ...a, acknowledged: true } : a)
    );
  }, []);
