        ...
        <div className="p-[32px] overflow-y-auto max-h-[70vh] flex flex-col gap-[20px]">
          ...
          <div className="grid grid-cols-2 gap-[16px]">
            ...
            <div className="flex flex-col gap-[8px]">
              ...
              {/* PENDING is set by the system, never by the admin. While the
                  guardian has not set their password the field is locked at
                  PENDING; setPassword() moves it to ACTIVE on its own and the
                  control unlocks. The backend enforces the same rule. */}
              <select
                value={editGuardian.status}
                onChange={(e) => onChange('status', e.target.value)}
                disabled={!editGuardian.isPasswordSet}
                className="w-full p-[14px] bg-white dark:bg-slate-900 border-[1.5px] border-[#cbd5e1] dark:border-slate-600 rounded-[8px] font-bold text-[#00212e] dark:text-white text-[0.95rem] outline-none cursor-pointer focus:border-[#00a8e8] dark:focus:border-[#38bdf8] disabled:cursor-not-allowed disabled:opacity-60 disabled:bg-[#f1f5f9] dark:disabled:bg-slate-800 appearance-none bg-[url('data:image/svg+xml,%3Csvg_xmlns=%22http://www.w3.org/2000/svg%22_width=%2212%22_height=%228%22_viewBox=%220_0_12_8%22%3E%3Cpath_fill=%22%2300212e%22_d=%22M1_1l5_5_5-5%22/%3E%3C/svg%3E')] dark:bg-[url('data:image/svg+xml,%3Csvg_xmlns=%22http://www.w3.org/2000/svg%22_width=%2212%22_height=%228%22_viewBox=%220_0_12_8%22%3E%3Cpath_fill=%22%23ffffff%22_d=%22M1_1l5_5_5-5%22/%3E%3C/svg%3E')] bg-no-repeat bg-[right_14px_center]"
              >
                {!editGuardian.isPasswordSet && <option value="PENDING">PENDING</option>}
                <option value="ACTIVE">ACTIVE</option>
                <option value="INACTIVE">INACTIVE</option>
              </select>
              {!editGuardian.isPasswordSet && (
                <span className="text-[0.75rem] font-semibold text-[#64748b] dark:text-slate-400">
