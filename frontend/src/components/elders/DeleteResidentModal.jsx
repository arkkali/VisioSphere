...
const DeleteResidentModal = ({ deleteTargetIds, residents, getFullName, onConfirm, onCancel }) => {
  return (
    <div
    ...
    >
      <div
        className="bg-white dark:bg-slate-800 rounded-[14px] shadow-2xl w-[90%] max-w-[450px] max-h-[90vh] overflow-y-auto animate-[slideUp_0.28s_ease] transition-colors duration-300"
      ...
      >
          ...
          <p className="text-[#2E3A59] dark:text-slate-300 text-[1rem] leading-[1.6] m-0 text-center font-medium">
            ...
            <strong className="text-[#00212e] dark:text-white">
              {deleteTargetIds.size === 1
                ? getFullName(residents.find((r) => deleteTargetIds.has(r._id)))
                : `${deleteTargetIds.size} residents`}
            </strong>
        ...
        <div className="flex justify-end gap-[12px] p-[20px_24px] border-t border-[#e2e8f0] dark:border-slate-700 bg-[#f8fafc] dark:bg-slate-900/50 rounded-b-[14px]">
          <button
            className="flex-1 p-[12px_24px] bg-white dark:bg-slate-800 border-[1.5px] border-[#cbd5e1] dark:border-slate-600 text-[#475569] dark:text-slate-300 rounded-[8px] font-bold hover:bg-[#e2e8f0] dark:hover:bg-slate-700 transition-colors"
            onClick={onCancel}
          >
            Cancel
          </button>
          <button
            className="flex-1 p-[12px_28px] border-none bg-gradient-to-br from-[#e11d48] to-[#be123c] dark:from-rose-600 dark:to-rose-800 text-white rounded-[8px] font-bold shadow-[0_4px_12px_rgba(225,29,72,0.25)] hover:-translate-y-[2px] transition-all"
            onClick={onConfirm}
          >
            Yes, Delete
          </button>
